...
def form_trapezoid_centers(array):
    ...
    return new_array
# Fix digits
# # JS like function analog
def toFixed(number, digits = 4):
...
def create_thcond(array):
    ...
    for i in range(0, np.shape(array)[i]):
        ...
        thcond.write('\n')
        thcond.write(str(toFixed(array[i][0][1]))+','+str(toFixed(array[i][2][1])))
        thcond.write('\n')
        thcond.write(str(toFixed(array[i][0][0]))+','+str(toFixed(array[i][1][0]))+','+str(toFixed(array[i][2][0]))+','+str(toFixed(array[i][3][0])))
        thcond.write('\n')
        thcond.write('-\n')
        thcond.write('-\n')
    thcond.close()        
...
def thcond_to_np_array():    
    ...
    data = data[:i]
    print(data)
    Array1 = np.zeros((len(data)//5,6), float)
    i = 0
    j = 0
    Temp = ''
    for line in data:
...
def run_pygame():    
    ...
    while loop:
        ...
        for i in range(0, np.shape(array)[0]):
            pygame.draw.line(screen, BLACK, [Array_V[i][0][0], Array_V[i][0][1]], [Array_V[i][1][0], Array_V[i][1][1]], 1)
            pygame.draw.line(screen, BLACK, [Array_V[i][1][0], Array_V[i][1][1]], [Array_V[i][2][0], Array_V[i][2][1]], 1)
            pygame.draw.line(screen, BLACK, [Array_V[i][2][0], Array_V[i][2][1]], [Array_V[i][3][0], Array_V[i][3][1]], 1)
            pygame.draw.line(screen, BLACK, [Array_V[i][3][0], Array_V[i][3][1]], [Array_V[i][0][0], Array_V[i][0][1]], 1)
        ...
        #Visualising axis
        pygame.draw.line(screen, RED, [Axis_V[0][0], Axis_V[0][1]], [Axis_V[1][0], Axis_V[1][1]], 1)
        pygame.display.flip()
    clock.tick(60)
    pygame.quit()
